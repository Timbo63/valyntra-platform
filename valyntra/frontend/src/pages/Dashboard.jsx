    ...
    <div className="page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <select
            value={selected?.id || ''}
            onChange={e => setSelected(companies.find(c => c.id === parseInt(e.target.value)))}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border)' }}>
                    {['Provider','Use Case','Match Score','Cap. Match','Ind. Match','Est. Value','Stage'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '8px 12px', fontSize: 12, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.5 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
