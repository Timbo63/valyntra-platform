...
export default function Assessment() {
  ...
  const [step, setStep]       = useState(0) // 0=company, 1-4=questions, 5=details, 6=done
          ...
          <>
            ...
            {companies.length > 0 && !isNew && (
              <>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--muted)', display: 'block', marginBottom: 6 }}>Select existing company</label>
            ...
            {isNew && (
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                <div><label style={labelStyle}>Company Name *</label><input value={company.name} onChange={e=>setCompany({...company,name:e.target.value})} placeholder="Acme Corp" /></div>
                ...
                <div className="grid-2">
                  <div><label style={labelStyle}>County</label><input value={company.county} onChange={e=>setCompany({...company,county:e.target.value})} placeholder="Broward" /></div>
                  <div><label style={labelStyle}>Employees</label><input type="number" value={company.employee_count} onChange={e=>setCompany({...company,employee_count:e.target.value})} placeholder="250" /></div>
                </div>
                <div><label style={labelStyle}>Primary Business Function</label><input value={company.primary_function} onChange={e=>setCompany({...company,primary_function:e.target.value})} placeholder="Operations, Sales, etc." /></div>
              </div>
            )}
        ...
        {step === 5 && (
          <>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              <div>
                <label style={labelStyle}>Primary operational pain point</label>
              ...
              <div>
                <label style={labelStyle}>Current tools in use</label>
...
const labelStyle = { fontSize: 13, fontWeight: 600, color: 'var(--muted)', display: 'block', marginBottom: 6 }
